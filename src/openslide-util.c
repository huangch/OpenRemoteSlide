 ...
 *  Copyright (c) 2019 Chao-Hui HUANG
 *  All rights reserved.
 *
...
#undef fopen
static URLIO_FILE *do_fopen(const char *path, const char *mode, GError **err) {
	URLIO_FILE *f;
...
URLIO_FILE *_openslide_fopen(const char *path, const char *mode, GError **err)
{
  ...
  URLIO_FILE *f = do_fopen(path, m, err);
  g_free(m);
  if (f == NULL) {
    return NULL;
  }
